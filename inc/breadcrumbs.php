...
function wayup_get_breadcrumbs() {
    ...
    $shop_product_short_title=get_the_title();

    if(get_post_meta(get_the_ID(),'wayup_short_title',true)){
           $shop_product_short_title= get_post_meta(get_the_ID(),'wayup_short_title',true);
        } else {
            $shop_product_short_title= get_the_title(); 
        }
   /* fix search mistake*/
    ...
    if (is_home() || is_front_page()) {
        ...
        if ( is_category() ) {
            ...
            if( get_post_type() == 'product') {
                ...
                printf($link, $home_link . '/' . $slug['slug'] . '/', 'Магазин'/*$post_type->labels->singular_name*/);
                if ($show_current == 1) echo $delimiter . $before . $shop_product_short_title . $after;
            }
